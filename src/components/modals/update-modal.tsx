...
const UpdateModal = ({ isUpdateOpen, setIsUpdateOpen, worker }: UpdateModalProps) => {
    const [selectedFile, setSelectedFile] = useState("");
    const { updateWorker } = useDataStore();
    ...
    useEffect(() => {
        setSelectedFile(worker?.imageUrl || "")
        form.setValue("name", worker?.name || "")
        form.setValue("plan", worker?.plan || "")
    }, [worker]);
    ...
    function onSubmit(data: z.infer<typeof UserSchema>) {
        ...
        updateWorker(worker?.id as string, newWorker);
        form.reset()
        setSelectedFile("")
